(function(){
  ...
  QUnit.test("Variables created with var in a funtion are re-created each time", function(assert){
    ...
    yay();
    assert.equal(this.counter, undefined);//counter is not defined until the else statement so the value never changes.
    yay();
  ...
  QUnit.test("Inner scope can access outer scope", function(assert){
    ...
    assert.equal(yay(), "alphaomega");//returns the value of im_outside concatenated with the vlaue of im_inside.
  });
  ...
  QUnit.test("Functions retain outer scope references between calls.", function(assert){
    ...
    yay();
    assert.equal(im_outside, 14);//Changes the value every time you call the function.
    yay();
  ...
  QUnit.test("We can do goofy stuff with outer scope", function(assert){
    ...
    yay();
    assert.equal(name, "greg");//Increments the value of name by the value og hello each time the function is called.
    yay();
  ...
  QUnit.test("We can pass functions to other functions and then run them.", function(assert){
    ...
    something(yay);
    assert.equal(im_outter, 40);//Passing the outcome of the yay function and passing that as an argument for when we call something and re-assigns value to im_outer.
  ...
  QUnit.test("We can get crazy with returns.", function(assert){
    ...
    assert.equal(foo(yay), "hello, this is dog");//Passes the return from yay() as the argument for foo(whatever) which concatenates the strings.
  });
